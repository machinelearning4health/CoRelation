...
class IcdModel(nn.Module):
    ...
    def forward(self, batch, rdrop=False, indices=None):
        if rdrop:
            ...
            return self.forward_normal(batch, indices)

    def forward_normal(self, batch, indices):
        ...
        if self.topk_num > 0:
            if indices is None:
                with torch.no_grad():
                    label_feats = self.train_calculate_text_hidden()
                    c_logits, c_alphas = self.decoder(hidden.detach(), word_mask, label_feats, term_count)
            ...
            c_logits, c_alphas = self.decoder(hidden, word_mask, label_feats, term_count, indices,
            ...
            if self.args.break_loss:
                ...
                if 'alpha_weight' in self.loss_config and self.loss_config[
                    'alpha_weight'] != 0.0 and c_alphas is not None and c_alphas is not None:
                    a_loss = (c_alphas.mean() * 0.5 + c_alphas.mean() * 0.5)
            else:
                avg_label_num = self.args.sample_num * 2
                c_loss = loss_fn(c_logits, c_label, self.loss_config, avg_label_num=avg_label_num)
                if 'alpha_weight' in self.loss_config and self.loss_config[
                    'alpha_weight'] != 0.0 and c_alphas is not None:
                    a_loss = (c_alphas.sum() / c_alphas.shape[0] / avg_label_num)
        else:
            if self.args.random_sub:
                c_logits, c_alphas = self.decoder(hidden, word_mask, label_feats, 1, mlabel_feat=label_feats_m)
            else:
                c_logits, c_alphas = self.decoder(hidden, word_mask, label_feats, self.args.term_count, mlabel_feat=label_feats_m)
            # mc_label = batch[-1]
            c_label = batch[-2]
            c_loss = loss_fn(c_logits, c_label, self.loss_config)
        ...
        return {'mc_loss': mc_loss, 'c_loss': c_loss * self.loss_config['code_loss_weight'], 'loss': loss, 'alpha_loss': self.loss_config['alpha_weight']*a_loss, 'indices_next': indices_next}
    ...
    def forward_rdrop(self, batch, indices):
        ...
        if self.topk_num > 0:
            if indices is None:
                ...
                topk_num = self.args.sample_num if label_feats_m is None else self.topk_num
                indices_t = self.select_traget_codes(c_logits, topk_num, batch[-2])
                indices_all = self.select_random_codes(2*topk_num-indices_t.shape[0], batch[-2], indices_top=indices_t)
                if self.args.use_graph:
                    if self.args.break_loss:
                        indices_next = self.select_traget_codes(c_logits, self.topk_num, batch[-2])
                    else:
            ...
            # [V*T, E]
            label_feats = self.train_calculate_text_hidden(indices)
            c_logits0, c_alphas0 = self.decoder(hidden0, word_mask, label_feats, term_count, indices, mlabel_feat=label_feats_m)
            c_logits1, c_alphas1 = self.decoder(hidden1, word_mask, label_feats, term_count, indices, mlabel_feat=label_feats_m)
            c_label = batch[-2][:, indices]
            if self.args.break_loss:
        ...
        if a_loss is not None:
            ...
            a_loss = torch.FloatTensor([0])
        return {'kl_loss':self.loss_config['rdrop_alpha'] * kl_loss, 'c_loss': c_loss * self.loss_config['code_loss_weight'], 'loss':loss, 'alpha_loss': self.loss_config['alpha_weight']*a_loss,
    ...
    def predict(self, batch, threshold=None, return_attentions=False):
        ...
        if self.topk_num > 0 and self.args.use_graph:
            ...
            if return_attentions:
                yhat_raw_fine, alphas, attentions = self.decoder(hidden, word_mask, label_feats, self.args.term_count, indices,
                                                            mlabel_feat=self.mlabel_feats,
                                                            return_attentions=return_attentions)
            else:
                yhat_raw_fine, alphas = self.decoder(hidden, word_mask, label_feats, self.args.term_count, indices, mlabel_feat=self.mlabel_feats)
            ...
            assert hasattr(self, 'label_feats')
            if return_attentions:
                yhat_raw, alphas, attentions = self.decoder(hidden, word_mask, self.label_feats, self.args.term_count, mlabel_feat=self.mlabel_feats, return_attentions=return_attentions)
            else:
                yhat_raw, alphas = self.decoder(hidden, word_mask, self.label_feats, self.args.term_count, mlabel_feat=self.mlabel_feats)
            if isinstance(yhat_raw, tuple):
                yhat_raw = yhat_raw[0]
            if threshold is None:
                threshold = self.args.prob_threshold
